...
hosted 埋め込みモデル `BAAI/bge-m3`（dense・1024 次元）のインデックスを Upstash（Free プラン）で
作成し、`src/storage/vectorStore.ts` と同じ **namespace 付き・data ベース（サーバー側埋め込み）**の
upsert / query / fetch が無料枠で成立するか。

> 一時は Upstash コンソールに bge-m3 が出ず openai/text-embedding-3-small で検証したが、
> **Vercel の Upstash 統合からは BGE_M3 を選べる**と判明し当初設計へ復帰。詳細は
...
1. [Upstash Console](https://console.upstash.com/) または **Vercel の Upstash Vector 統合**
   （Install Integration）でインデックスを作成。
2. **Create Index** で:
   - **Type**: `Dense`
   - **Embedding Model**: `BGE_M3` を選択（← 最重要。多言語=日本語対応・1024 次元・埋め込みは
     Upstash 内で完結＝OpenAI を経路に挟まない）
   - **Similarity**: `COSINE`
   - **Region**: 任意（個人用途なら近いリージョン）
   - **Plan**: **Free**（PAYG/Fixed を選ばない。Free は 1 index までだが本機能は 1 index + 4 namespace で充足）
...
> 注: このスパイクが PASS でも証明できるのは**機能経路**（hosted bge-m3 埋め込み＋namespace＋
> data ベース I/O が動く）だけ。Free 枠の容量/QPS 上限は別途確認する。

## 失敗パターンと示唆

| 症状 | 示唆 |
|---|---|
| `dimension=0` / data upsert で「requires dense vectors」系エラー | 埋め込みモデル無し（Custom）のインデックス。作り直して `BGE_M3`（dense）を選ぶ |
| インデックス作成時に課金/カード必須で詰まる | hosted bge-m3（Free）の前提が崩れる。research.md R1 の代替案（Custom + ローカル埋め込み）を再検討 |
