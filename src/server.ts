...
export function createMcpServer(deps: McpServerDeps): McpServer {
	...
	server.registerTool(
		...
		{
			title: "ワインを記録する",
			description: "確認済みのワイン記録を永続化する。**ユーザーが内容を明示的に承認した後にのみ呼ぶこと**" + '（保存前の確認は preview_record で行う）。name は必須・非空、color は "white" または "red"、' + "各 *Terms は当該 color の JSA 語彙内のみ、imageUrl は自ストレージの https のみ。" + "検証に失敗した場合は保存せず、フィールド別エラーを返す。",
	...
	server.registerTool(
		"preview_record",
		{
			...
			description: "抽出した下書きを正規化・検証し、保存される内容をテキストで返す（読み取り専用・保存しない）。" + "record_wine の前にこれで内容を提示し、ユーザーの明示承認を得ること。",
			inputSchema: recordWineInputSchema,
		},
		async (args): Promise<CallToolResult> => {
	...
	server.registerTool(
		"get_jsa_taxonomy",
		{
			...
			description: "外観・香り・味わいの定義済み表現語彙（JSA テイスティング用語）を返す。" + 'color は "white" または "red"（必須。白用/赤用で語彙が異なる）。category 指定で該当カテゴリのみ。' + "サブカテゴリ（清澄度・色調…）ごとに selectCount（試験での選択数の目安・参考情報）と terms を返す。" + "ユーザーはこの語彙からのみ表現を選ぶ（record_wine は語彙外を拒否する）。",
			inputSchema: {
				color: z.string().optional().describe('"white" または "red"（必須）'),
