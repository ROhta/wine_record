...
抽出した下書きを**確認ウィジェット（MCP Apps UI リソース）**として返す。保存はしない
（明示承認＝保存の引き金は record_wine 側）。

- **入力**: `record_wine` と同形の下書き（**全フィールド任意・nullable**）。vision 抽出値を
  そのまま渡す。空でもよい（空フォームを返す）。
- **出力**: 確認ウィジェットの **UI リソース（EmbeddedResource, HTML）**。下書き値を埋め込んで
  フォームに事前入力する。
- **副作用**: なし（保存しない）。
- **承認フロー（FR-003）**: ユーザーがウィジェットで修正・`color` 選択・**承認**すると、
  ウィジェットがブリッジ経由で **`record_wine` を直接呼び出して永続化**する。モデルに依存せず
  明示承認を構造的に保証する。
- **備考**: 正確な mimetype / UI リソース URI / ブリッジ API は、描画検証スパイク（T022a）と
  `build-mcp-app` の作法で確定する。
