...
外観・香り・味わいの定義済み表現語彙を返す（表現選択の供給源）。白用・赤用で用語セットが
異なるため **`color` 必須**。サブカテゴリ（清澄度・色調・第一印象…）単位でグループ化して返す。

> リモート HTTP では確認ウィジェット（タップ UI）が使えないため、モデルがこの語彙を
> カテゴリ別に提示 → ユーザーがチャットで選択 → `record_wine`/`preview_record` が語彙外を
> 拒否する、というテキストベースの選択フローで使う（research.md R5）。

- **入力**: `{ "color": "white" | "red", "category"?: "appearance" | "aroma" | "taste" }`
  （`color` 必須。`category` 省略時は 3 カテゴリすべて）
- **出力**:
  ```json
  {
    "color": "white",
    "version": "2020.01.11",
    "appearance": [
      { "name": "清澄度", "selectCount": 1, "terms": ["澄んだ", "..."] }
    ],
    "aroma": [{ "name": "第一印象", "selectCount": 2, "terms": ["..."] }],
    "taste": [{ "name": "アタック", "selectCount": 1, "terms": ["..."] }]
  }
  ```
  `category` 指定時は該当カテゴリのみ返す。`selectCount` は JSA 試験での選択数の目安
  （**参考情報**。検証上の制約ではない。`record_wine` の検証は語彙所属のみで件数は強制しない）。
- **検証**: `color` が "white"/"red" 以外、または `category` が不正ならフィールド別エラーを返す。
- **副作用**: なし（読み取りのみ）。
