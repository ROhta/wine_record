...
| `token_endpoint_auth_method` | クライアント認証方式 | コード管理 | 本番値に合わせる（例 `client_secret_post`） |
| `client_secret` | クライアント秘密 | **管理外**（provider resource から削除済み・data source のみ） | output しない＝state 非露出。ローテーションしない |
